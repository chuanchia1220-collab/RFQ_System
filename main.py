    ...
    def run_analysis(self, e):
        ...
        try:
            ...
            for mat_type, group_items_list in grouped_items.items():
                ...
                card = ft.Card(content=ft.Container(padding=20, content=ft.Column([ft.Row([ft.Icon(ft.Icons.CATEGORY, color=ft.Colors.BLUE), ft.Text(f"材質群組: {mat_type}", size=20, weight=ft.FontWeight.BOLD)]), ft.Divider(), ft.Row([items_table], scroll=ft.ScrollMode.AUTO), ft.Divider(), ft.Text("選擇詢價對象 (最多 4 家):", weight=ft.FontWeight.BOLD), ft.Row(supp_dds, wrap=True), ft.Row([batch_draft_btn], alignment=ft.MainAxisAlignment.END)])))
